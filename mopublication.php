...
$memberKey = 'lfxPu9TiUCLdWGTIhkei';
...
function moPubAddPage() {
    ...
    $newPage = array(0 => array("name" => $page->post_title, "type" => "page", "link" => mopub_getPermalinkStructure()."type=mobile&article_id=".urlencode($page->ID), "ID" => $page->ID, "icon" => $_POST['itemIcon']));
    
    mopub_addToTabsOrder($newPage);
...
function mopubDeletePage() {
    ...
    mopub_removeTabFromOrder('ID', $_POST['ID']);
...
function mopub_settings_print_scripts()
{
    ...
    $timestamp = time();
    wp_localize_script('mopub-interface', 'WPULRS', array( 'siteurl' => get_option('siteurl'), 'permalinkstructure' => mopub_getPermalinkStructure(), 'key' => md5(get_option('mopub_salt').$timestamp), 't' => $timestamp ));
...
function deactivateMoPublication() {
    ...
    if(function_exists('file')) {
        ...
        $output = file('http://www.mopublication.com/members/api/mopub-plugin/plugin-deactivation-notification?_key='.$memberKey.'&url='.$wordpressURL);
