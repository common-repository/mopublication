    ...
    <div class="frm_right">
        ...
        <ul id="pagessortable">
            <?php
             ...
             $systemTabs = mopub_getSystemTabs();   
             $tabOrder = json_decode(get_option('mopub_tabs_order'), true);
             $oldSystemTabs = get_option('old_system_tabs');
             $systemTabsFlag = 0;
             
             if(empty($oldSystemTabs)) {
                 
                 update_option( 'old_system_tabs', $systemTabs );
                 $oldSystemTabs = get_option('old_system_tabs');
                 
             }
             
             //A new option has been added
             if($systemTabs != $oldSystemTabs) {
             ...
             if(empty($tabOrder)) {
                ...
                $tabOrder = mopub_addToTabsOrder($systemTabs);
                 
             } 
             
             if($systemTabsFlag == 1) {

                 update_option('mopub_tabs_order', '');
                 $tabOrder = mopub_addToTabsOrder($systemTabs);
